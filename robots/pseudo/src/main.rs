...
#[derive(Debug, Parser)]
#[command(name = "rollio-robot-pseudo")]
#[command(about = "Synthetic robot driver for Sprint 2")]
struct Cli {
    #[command(subcommand)]
    command: Command,
}
...
fn main() -> Result<(), Box<dyn Error>> {
    let cli = Cli::parse();
    match cli.command {
        Command::Probe { count, dof } => {
            let devices: Vec<_> = (0..count)
                .map(|index| {
                    json!({
                        "id": format!("pseudo_robot_{index}"),
                        "name": format!("pseudo_robot_{index}"),
                        "driver": "pseudo",
                        "type": "robot",
                        "dof": dof,
...
fn load_device_config(args: &RunArgs) -> Result<DeviceConfig, Box<dyn Error>> {
...
fn run_device(device: DeviceConfig) -> Result<(), Box<dyn Error>> {
