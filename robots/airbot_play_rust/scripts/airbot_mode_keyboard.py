...
"""Keyboard test UI for a running `rollio-device-airbot-play` process.

This script talks directly to the AIRBOT channel-local iceoryx2 services:
...
Usage examples:
  ...
  uv run robots/airbot_play_rust/scripts/airbot_mode_keyboard.py \
      --bus-root airbot_play \
      --channel arm \
      --channel g2
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="AIRBOT mode keyboard test UI")
    parser.add_argument("--bus-root", required=True, help="Device bus_root to inspect/control.")
    parser.add_argument(
        "--channel",
        action="append",
        dest="channels",
        default=[],
        help="Channel type to monitor/control. Repeat for multiple channels. Default: arm",
    )
    parser.add_argument(
        "--refresh-ms",
        type=int,
...
def open_channel_view(iox2: Any, node: Any, bus_root: str, channel_type: str) -> ChannelView:
    ...
    state_kinds = ARM_STATE_KINDS if channel_type == "arm" else EEF_STATE_KINDS
    state_subscribers: dict[str, Any] = {}
...
def render(bus_root: str, channels: list[ChannelView], selected_index: int) -> None:
    ...
    print("AIRBOT Channel Keyboard Test")
